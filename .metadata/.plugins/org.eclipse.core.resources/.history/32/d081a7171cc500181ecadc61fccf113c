...
public class CidadeResource {
	...
	public ResponseEntity<Cidade> criar(@Valid @RequestBody Cidade cidade, HttpServletResponse response) throws APIException, ParseException {
		...
		String nomeCidade = cidadeService.verificarDadosClima(cidade.getNm_Cidade(), cidade.getSg_Pais());
		...
		return !StringUtils.isEmpty(nomeCidade) ? ResponseEntity.status(HttpStatus.CREATED).body(cidadeSalva):ResponseEntity.notFound().build();
